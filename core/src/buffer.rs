...
//! ## 架构定位
//!
//! ShmBridge 是 QTS V8 **数据底座（L1-L2）** 的跨语言零拷贝通信层。
//! Rust 侧（FeatureEngine）将每 tick 计算的 50 维特征写入共享内存环形缓冲区，
//! Python 侧通过 DLPack 协议以 **零拷贝** 方式直接读取为 PyTorch / NumPy 张量。
//!
//! ## 数据流
//!
//! ```text
//! ┌──────────────────────────────────────────────────────────────┐
//! │ Rust (FeatureEngine)                                        │
//! │   on_tick(snapshot) → compute_50d_features()                │
//! │     │                                                       │
//! │     └──→ ShmBridge.push_snapshot(ts_ms, features[50])       │
//! │             │                                               │
//! │             └── 环形写入 mmap 区域 (720KB /dev/shm)         │
//! ├──────────────────────────────────────────────────────────────┤
//! │ Python (Research / AI)                                      │
//! │   dlpack = bridge.get_window(secs=60)                       │
//! │     │                                                       │
//! │     ├── torch.from_dlpack(dlpack) → Tensor(N, 50) 零拷贝   │
//! │     │                                                       │
//! │     └── ptr, shape = bridge.get_raw_ptr(secs=30)            │
//! │           └── np.frombuffer(CType(ptr, shape)) → ndarray    │
//! └──────────────────────────────────────────────────────────────┘
//! ```
//!
//! ## 平台适配
...
//! | 平台    | 共享内存实现                         | 路径/名称                   |
//! |---------|--------------------------------------|-----------------------------|
//! | Linux   | `memmap2` 映射 `/dev/shm/qts_eth5m`  | `/dev/shm/qts_eth5m`       |
//! | Windows | `Vec<f32>` 后备存储（可升级为 `CreateFileMapping`） | 命名共享内存 `qts_eth5m` |
//!
//! ## DLPack 协议
//!
//! 遵循 DLPack 0.8 规范，通过 `PyCapsule` 传递 `DLManagedTensor*`。
//! Python 侧的 `torch.from_dlpack` 和 `cupy.from_dlpack` 通过 `deleter` 回调
//! 安全释放 Rust 分配的 shape 数组。
//!
//! ## 性能契约
//!
//! | 操作              | 目标延迟   | 机制                                |
//! |-------------------|------------|-------------------------------------|
//! | `push_snapshot()` | < 1µs      | 零分配写入 mmap 区域（固定偏移量）  |
//! | `get_window()`    | < 2µs      | DLPack capsule 指针传递（无拷贝）   |
//! | `get_raw_ptr()`   | < 1µs      | 裸指针返回 + shape tuple            |
...
//! ## 安全约定
//!
//! 1. `ShmBridge()` 构造时自动创建/连接 SHM，无参构造
//! 2. `get_window()` 和 `get_raw_ptr()` 仅在 Rust backend 读锁持有期间返回指针
//! 3. DLPack capsule 的 `deleter` 回调会释放 shape 数组，Python GC 保证调用
...
/// DLPack 张量元数据（胖指针 + shape/strides）
///
/// ### 字段说明
///
/// - `data`: 数据起始地址
/// - `ndim`: 维度数（2，即 (N, 50)）
/// - `shape`: 形状数组指针（由 Rust Box 管理生命周期）
/// - `strides`: 步长数组（null 表示紧凑排列）
/// - `byte_offset`: 数据起始偏移（通常为 0）
#[repr(C)]
struct DLTensor {
    /// 数据起始地址
...
/// DLPack 托管张量（含析构器）
///
/// 通过 `PyCapsule` 传递给 Python，`deleter` 回调在 Python GC 回收
/// capsule 时被调用，释放 Rust 分配的 shape 数组。
#[repr(C)]
struct DLManagedTensor {
    /// 张量元数据
    dl_tensor: DLTensor,
    /// 托管上下文（指向 shape 数组的 Box，用于 deleter 释放）
    manager_ctx: *mut std::ffi::c_void,
    /// 析构回调（Python GC 触发）
    deleter: Option<unsafe extern "C" fn(*mut DLManagedTensor)>,
}

/// Wrapper to make *mut DLManagedTensor Send-safe for PyCapsule
struct DLTensorSend(*mut DLManagedTensor);
unsafe impl Send for DLTensorSend {}
...
/// 每快照的特征维度数
///
/// 固定为 50 维：价格变化率 × 成交量剖面 × OBI/OFI × 滑窗统计量
pub const FEATURE_DIM: usize = 50;

/// 环形缓冲区容量（1 小时 @ 1 秒/快照）
///
/// 写满后覆盖最旧数据（FIFO 循环覆盖策略）
...
/// Linux: mmap 文件路径
#[cfg(target_os = "linux")]
const SHM_PATH: &str = "/dev/shm/qts_eth5m";

/// Windows: 命名共享内存标识符
#[cfg(target_os = "windows")]
const SHM_NAME: &str = "qts_eth5m";
...
/// - `file.set_len(SHM_SIZE)` 预分配空间
/// - `MmapMut::map_mut()` 建立虚拟地址映射
/// - 环形写入通过 `head` 指针 + 取模实现
///
/// ### 性能
///
/// mmap 直写物理内存页，绕过文件系统页缓存，单次 push_row 约 200ns
#[cfg(target_os = "linux")]
mod shm_impl {
    use super::*;
    use memmap2::MmapMut;
    ...
    /// Linux SHM 后端
    ///
    /// `mmap` 指向 /dev/shm/qts_eth5m 的映射区域，
    /// `timestamps` 在堆上分配以避免 mmap 中存储元数据。
    ...
    impl ShmBackend {
        ...
        pub fn new() -> std::io::Result<Self> {
            ...
            // Safety: mmap 建立虚拟地址映射，运行时保证不超出 SHM_SIZE 范围
        ...
        /// ### 参数
        ///
        /// - `ts_ms: i64` — Unix 毫秒时间戳
        /// - `row: &[f32]` — 50 维特征（必须为 FEATURE_DIM 长度）
        ///
        /// ### 性能
        ///
        ...
        /// ### 并发安全
        ///
        /// 调用方必须持有 `write` 锁（由 ShmBridge.push_snapshot 保证）
        ...
        /// ### 算法
        ///
        /// 从最新时间戳反向遍历，找到第一个早于 cutoff 的条目，前面的跳过。
        /// 因为环形缓冲区按时间戳单调递增（覆盖后破坏单调性，
        /// 但相邻条目间差距 > BUFFER_CAPACITY 才可能，1h 内不会发生）。
...
/// ### 线程安全
///
/// 使用 `Arc<RwLock<ShmBackend>>` 封装后端，允许多个 Python 线程
/// 同时读（`get_window` / `get_raw_ptr`），写（`push_snapshot`）独占锁。
/// 生产场景下仅 Rust 侧单写，锁争用可忽略。
#[pyclass]
...
#[pymethods]
impl ShmBridge {
    /// 构造 ShmBridge 并自动连接共享内存
    ///
    /// ### 平台行为
    ///
    /// | 平台    | 操作                                           |
    /// |---------|------------------------------------------------|
    /// | Linux   | 打开/创建 `/dev/shm/qts_eth5m` → mmap 映射     |
    /// | Windows | 分配 Vec 后备存储（可升级为 CreateFileMapping） |
    ///
    /// ### 错误
    ///
    /// - `IOError`: 共享内存创建/映射失败（如路径权限不足）
    #[new]
    pub fn new() -> PyResult<Self> {
        let backend = ShmBackend::new().map_err(|e| {
    ...
    /// ### 参数
    ///
    /// - `ts_ms: int` — Unix 毫秒时间戳
    /// - `features: list[float]` — 50 维特征向量
    ///
    /// ### 错误
    ///
    /// - `ValueError`: features 长度不等于 50
    ///
    /// ### 性能
    ///
    /// O(50) 字节拷贝，~200ns（L1 缓存命中时）。不分配堆内存
    /// （除 RwLock 锁开销外）。
    ///
    /// ### 并发
    ///
    /// 获取写锁后写入，与其他 `get_window` / `get_raw_ptr` 读者互斥。
    /// 正常使用时 FeatureEngine 在单线程中调用，锁争用可忽略。
    pub fn push_snapshot(&self, ts_ms: i64, features: Vec<f32>) -> PyResult<()> {
    ...
    /// 获取最近 secs 秒的数据窗口 → DLPack 胶囊
    ///
    /// ### 参数
    ///
    /// - `secs: int` — 时间窗口（秒），如 60 = 最近 1 分钟
    ///
    /// ### 返回
    ///
    /// DLPack `PyCapsule`，可传递给 `torch.from_dlpack()` 实现零拷贝。
    /// 返回的 Tensor shape 为 `(N, 50)`，dtype 为 `float32`。
    ///
    /// ### Python 使用
    ///
    /// ```python
    ...
    /// ### 生命周期
    ///
    /// DLPack capsule 托管 shape 数组的释放。Python GC 回收 capsule 时
    /// 调用 `dlpack_deleter_with_shape` 释放 Rust Box。**注意**：释放时
    /// backend 可能已写入新数据，但 capsule 持有的指针仍有效（mmap 区域
    /// 不因数据更新而移动）。
    pub fn get_window<'py>(&self, py: Python<'py>, secs: u32) -> PyResult<Bound<'py, PyCapsule>> {
        let backend = self.backend.read().unwrap();
        let n = backend.recent_count_secs(secs as i64);

        // 构建 shape [N, 50]，用 Box 分配以保证 heap 稳定性
        let shape = Box::new([n as i64, FEATURE_DIM as i64]);
        let shape_ptr = shape.as_ptr();

        let data_ptr = backend.data_ptr() as *mut std::ffi::c_void;

        // Safety: managed tensor 包含指向 mmap 区域的指针和 Box 的 shape 指针。
        // mmap 区域由 MmapMut 持有（ShmBackend 存活期间有效）。
        // shape 数组由 Box 托管，通过 manager_ctx 传递给 deleter。
        let managed = Box::new(DLManagedTensor {
            dl_tensor: DLTensor {
                ...
                shape: shape_ptr,
                strides: std::ptr::null(), // null = 行优先紧凑排列
                byte_offset: 0,
            },
        ...
        PyCapsule::new_bound(py, DLTensorSend(raw), Some(std::ffi::CString::new("dltensor").unwrap()))
    }

    /// 获取最近 secs 秒数据的裸指针（NumPy 零拷贝兼容）
    ///
    /// ### 参数
    ///
    /// - `secs: int` — 时间窗口（秒）
    ...
    /// ).reshape(rows, cols)
    /// # 警告: 此 arr 不持有内存所有权，后端写入会导致数据变化
    /// ```
    ///
    /// ### 注意事项
    ///
    /// - 返回的指针指向 mmap 区域（或 Vec 堆内存），**不持有所有权**
    /// - Python 侧不应尝试释放此指针
    /// - 后端写入会直接修改指针指向的内存（无拷贝语义）
    pub fn get_raw_ptr(&self, secs: u32) -> (usize, (usize, usize)) {
        let backend = self.backend.read().unwrap();
        let n = backend.recent_count_secs(secs as i64);
    ...
    /// 获取最新一条快照 → MarketSnapshot proto JSON bytes
    ///
    /// ### 返回
    ///
    /// `bytes` — JSON 格式的 MarketSnapshot 数据，对齐 `schemas/market_snapshot.proto`
    /// 结构。Qwen 侧使用 `google.protobuf.json_format.Parse()` 反序列化。
    ///
    /// ### 结构
    ///
    /// ```json
    ...
    /// - 热路径应使用 `get_window()` 或 `get_raw_ptr()` 替代
    /// - 缓冲区为空时返回空 bytes `b""`（非 `None`）
    ///
    /// ### 返回
    ///
...
/// ### 调用时机
///
/// Python GC 回收 DLPack capsule 时（C 侧 `DLManagedTensor::deleter`）。
///
/// ### Safety
///
/// `ptr` 必须是由 `get_window()` 分配的 `Box<DLManagedTensor>`，
/// `manager_ctx` 必须指向 `Box<[i64; 2]>`。调用后两者均被释放。
unsafe extern "C" fn dlpack_deleter_with_shape(ptr: *mut DLManagedTensor) {
    if !ptr.is_null() {
    ...
    // managed (DLManagedTensor) 随 Box 析构自动释放
}
...
mod tests {
    ...
    fn test_shm_push_and_read() {
        let bridge = ShmBridge::new().unwrap();
        bridge
    ...
    fn test_recent_count_secs_filtering() {
        ...
        // 2s 窗口：仅包含 ts=2000（ts=1000 在窗口外）
        let (_, (rows, _)) = bridge.get_raw_ptr(2);
        assert_eq!(rows, 1);

        // 60s 窗口：两条都应包含
        let (_, (rows2, _)) = bridge.get_raw_ptr(60);
        assert_eq!(rows2, 2);
    }
